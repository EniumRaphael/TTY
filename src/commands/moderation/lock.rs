...
impl SlashCommand for Lock {
    fn name(&self) -> &'static str {
        "lock"
    }

    fn description(&self) -> &'static str {
        "Lock the channel (nobody can speak anymore)"
    }
    ...
    fn register(&self) -> CreateCommand {
        ...
        let channel: CreateCommandOption = CreateCommandOption::new(CommandOptionType::Channel, "channel", "The channel to lock")
            .required(false);
        options.push(channel);

        let notify: CreateCommandOption = CreateCommandOption::new(CommandOptionType::Boolean, "notify", "Will I send a message after locking the channel")
            .required(false);
        options.push(notify);
    ...
    async fn run(
        ...
        let _guild_id: GuildId = command.guild_id.ok_or(anyhow::anyhow!("Lock command executed in DM"))?;
        ...
        let message = CreateInteractionResponseMessage::new()
            .content(format!("{} | Channel gonna be locked", _emoji.answer.loading))
            .ephemeral(true);
        command.create_response(&ctx.http, CreateInteractionResponse::Message(message)).await?;
        ...
        if let Some(existing) = overwrites.iter_mut().find(|o| o.kind == PermissionOverwriteType::Role(everyone_role)) {
            existing.allow &= !Permissions::SEND_MESSAGES;
            existing.deny |= Permissions::SEND_MESSAGES;
        } else {
        ...
        let edit_msg = EditInteractionResponse::new()
            .content(format!("{} | Channel {} is now locked", _emoji.answer.yes, channel.name));
...
inventory::submit! {
    CommandEntry { create: || Box::new(Lock) }
}
