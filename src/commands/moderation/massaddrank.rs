...
pub struct MassAddRank;

#[serenity::async_trait]
impl SlashCommand for MassAddRank {
    fn name(&self) -> &'static str {
        "massaddrank"
    }

    fn description(&self) -> &'static str {
        "Adding the role to all people"
    }
    ...
    async fn run(
        ...
        let user_to_add: Vec<Member> = guild.members(&ctx.http, Some(guild.member_count), None).await?;
        let user_all_ready_ranked: Vec<Member> = get_members_with_role(&ctx, guild_id, target_id).await?;
        let user_to_add: Vec<Member> = user_to_add
            .into_iter()
            .filter(|m| !user_all_ready_ranked.iter().any(|r| r.user.id == m.user.id))
            .collect();

        let user_size: usize = user_to_add.len();
        if user_size == 0 {
            let message: CreateInteractionResponseMessage = CreateInteractionResponseMessage::new()
                .content(format!("{} | Cannot assign {} to anybody.", _emoji.answer.error, target.mention()))
                .ephemeral(true);
            let response: CreateInteractionResponse = CreateInteractionResponse::Message(message);
            command.create_response(&ctx.http, response).await?;
            return Ok(());
        ...
        let message: CreateInteractionResponseMessage = CreateInteractionResponseMessage::new()
            .content(format!("{} | Role {} gonna be added to {}", _emoji.answer.loading, target.mention(), msg))
            .ephemeral(true);
        let response: CreateInteractionResponse = CreateInteractionResponse::Message(message);
        command.create_response(&ctx.http, response).await?;

        for member in &user_to_add{
            member.add_role(&ctx.http, target_id).await?;
        }

        let edit_msg: EditInteractionResponse = EditInteractionResponse::new()
            .content(format!("{} | Role {} is added from {} users", _emoji.answer.yes, target.mention(), user_size));
...
inventory::submit! {
    CommandEntry { create: || Box::new(MassAddRank) }
}
