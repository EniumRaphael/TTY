...
impl SlashCommand for Hide {
    ...
    fn register(&self) -> CreateCommand {
        ...
        let channel: CreateCommandOption = CreateCommandOption::new(CommandOptionType::Channel, "channel", "The channel to Hide")
            .required(false);
        options.push(channel);

        let notify: CreateCommandOption = CreateCommandOption::new(CommandOptionType::Boolean, "notify", "Will I send a message after Hideing the channel")
            .required(false);
        options.push(notify);
    ...
    async fn run(
        ...
        let _guild_id: GuildId = command.guild_id.ok_or(anyhow::anyhow!("Hide command executed in DM"))?;
        ...
        let message = CreateInteractionResponseMessage::new()
            .content(format!("{} | Channel gonna be Hidden", _emoji.answer.loading))
            .ephemeral(true);
        command.create_response(&ctx.http, CreateInteractionResponse::Message(message)).await?;
        ...
        if notify {
            channel.say(&ctx.http, format!(
                "🙈 | This channel was hidden by {}",
                command.user.name
            )).await?;
        }

        let edit_msg = EditInteractionResponse::new()
            .content(format!("{} | Channel {} is now hidden", _emoji.answer.yes, channel.name));
...
inventory::submit! {
    CommandEntry { create: || Box::new(Hide) }
}
